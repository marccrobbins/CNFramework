...
//    }
	...
	void Update ()
    {
        ...
        else if(CurrentTeleportState == TeleportState.Selecting)
        {
            ...
            int index = 0;//(int)ActiveController.index;
            GameObject device = null;//SteamVR_Controller.Input(index);
            bool shouldTeleport = false;//device.GetPressUp(SteamVR_Controller.ButtonMask.Touchpad);
